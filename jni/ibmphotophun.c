...
JNIEXPORT void JNICALL Java_com_msi_ibm_ndk_MainActivity_Test(JNIEnv * env, jobject obj)
{
//int i;
//int w = width;
//
//jboolean isCopy;
//jint* yuv = (*env)->GetIntArrayElements(env, yuv420sp, &isCopy);
//
//for(i = 0; i < w; i++) {
////	data[i] = yuv[i] + 1;
//}
//
//(*env)->SetIntArrayRegion(env, rgb, 0, w, ( jint * ) &data[0] );
//
//(*env)->ReleaseIntArrayElements(env, yuv420sp, yuv, JNI_ABORT);
//	jint i = 2;
//	jstring js = new jstring(i);
